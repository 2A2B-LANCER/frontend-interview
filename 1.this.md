...
1. this指向函数自身（JavaScript中所有函数都是对象）
2. this的作用域

### 2. 需要知道的点

1. this是在运行时绑定的
2. this的绑定完全取决于该函数被谁调用（去[调用栈](https://developer.mozilla.org/zh-CN/docs/Glossary/Call_stack)中寻找）
...
bind 则是在调用的时候生成一个与原函数一模一样的函数，但是生成的函数有以下限制：

1. 函数调用的 this 只能绑定到bind的第一个参数，即便再次使用call，apply也无效；
2. bind 从第二个开始的参数就是该复制函数的指定参数值，复制函数调用输入的参数从未被指定的参数开始赋值
...
1. 箭头函数本身没有this，所以箭头函数中的this是从作用域链上找到的
2. 对箭头函数应用任何绑定this的技术都无效（因为箭头函数没有this）
