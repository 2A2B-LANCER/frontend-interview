...
!(function (a, b) {
  ...
  function f(a) {
    for (a = a.replace(F, "/"), a = a.replace(H, "$1/"); a.match(G); )
      a = a.replace(G, "/");
    return a;
  }
  function g(a) {
  ...
  function r() {
    ...
    for (
      var a = aa.getElementsByTagName("script"), b = a.length - 1;
      b >= 0;
      b--
    ) {
      var c = a[b];
  ...
  function s(a) {
    var b = [];
    return (
      a.replace(fa, "").replace(ea, function (a, c, d) {
        d && b.push(d);
      }),
      b
    );
  ...
  if (!a.seajs) {
    ...
    var M =
      ...
      O,
      P,
      Q = !location.href || N.test(location.href) ? "" : e(location.href);
    if (M) {
      var R;
      try {
        var S = Error();
    ...
    (t.prototype.resolve = function () {
      ...
      (t.prototype.load = function () {
        ...
        if (!(a.status >= la.LOADING)) {
          ...
          for (d = 0; e > d; d++)
            (g = ga[c[d]]),
              g.status < la.FETCHING
                ? g.fetch(f)
                : g.status === la.SAVED && g.load();
