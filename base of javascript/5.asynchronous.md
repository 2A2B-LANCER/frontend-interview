...
- 实现基本功能：
  - 执行了`resolve`，Promise状态会变成`fulfilled`
  - 执行了`reject`，Promise状态会变成`rejected`
...
- 第二轮渲染：宏任务队列有 `resolve();document.body.style = 'background:black'`；`resolve()`会把 `Promise的then() ` 添加到为任务队列，所以微任务队列有 `document.body.style = 'background:pink'`，所以第二轮渲染为粉色
